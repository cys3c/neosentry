...
function doTraceroute($device, $outFile){
	...
	$arrOldTraceroute = (file_exists($outFile))?json_decode(file_get_contents($outFile)):[];
    $oldTraceroute = $arrOldTraceroute["success"]["result"]; //used to detect route changes.
    $arrTraceroute = $arrOldTraceroute;
    ...
    $deviceIP = $device;
    if (ip2long($device) === FALSE) {
        $deviceIP = gethostbyname($device);
    }

    //if (strpos(end($rows), '*' === FALSE)) { //old comparison
    if ($lastIP == $deviceIP) {
        //successful traceroute. update the array
        $arrTraceroute["success"]["date-time"] = date(DATE_ATOM);
        $arrTraceroute["success"]["command"] = $trCmdIcmp;
        $arrTraceroute["success"]["result"] = $trRet;
        ...
        //if (strpos(end($rows), '*') === FALSE) { //old comparison
        if ($lastIP == $device) {
            //TCP scan was successful, lets update the array
            $arrTraceroute["success"]["date-time"] = date(DATE_ATOM);
            $arrTraceroute["success"]["command"] = $trCmdTcp;
            $arrTraceroute["success"]["result"] = $trRetTcp;
            ...
            $arrTraceroute["failed"]["date-time"] = date(DATE_ATOM);
            $arrTraceroute["failed"]["command"] = $trCmdTcp;
            $arrTraceroute["failed"]["result"] = $trRetTcp;
    ...
    file_put_contents($outFile,json_encode($arrTraceroute));
}

function updateLastHop($lastHop, $sDevice) {
	//if we have an IP then lets write it
	$lastHop = trim($lastHop);
	...
	if ($lastHop != "") {
		...
		return updateDeviceData($sDevice, "traceroute", array('lasthop'=>$lastHop));
	}
}
...
function compareTraceroutes($deviceToUpdate, $oTR, $nTR) {
	...
	for ($a=0;$a < (count($nt)>count($ot)?count($nt):count($ot));$a++) {
		...
		if ($ot[$a] != $nt[$a] || count($nt)!=count($ot)) {
			...
			writeLog("traceroute",$deviceToUpdate,"The network path to this device has changed. This could indicate a MITM attack.\n\nHop ".($a+1)." changed from {$ot[$a]} to {$nt[$a]}",$oTR, $nTR);
			break;
			}
		}
