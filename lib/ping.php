...
//variables
$ipList = ""; 
$ipListFile = "$scandataFolder/iplist"; //list of ip's to be used by fping
$pingFile = "$scandataFolder/current_ping.json"; //stores the current ping status results
$arrPingNew = ['date'=> date("Y-m-d H:i:s")];

// Create the device list with pingable IP's
$arrDevices = json_decode(file_get_contents("$dataFolder/devices.json"),true);
//echo '<pre>' . print_r($deviceList, true) . '</pre>';
...
//create the directory and put the ip list in the file
if (!is_dir($scandataFolder)) mkdir($scandataFolder, 0777, true);
file_put_contents($ipListFile, $ipList);

//run the command to get the ping results
...
foreach($rows as $row) {
    ...
    if ($sIp!="") {
        ...
        if (strrpos($row,"(")>0) {
            ...
            $file_tr = "$scandataFolder/traceroute_".$sIp."_fail";
